...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $Detalle_historial = "NO";//$request["detalle_historial"];
    $Id = (int) $request["id"];

    class Result {}
    $response = new Result();

    $query = "UPDATE  tbl_paciente SET nombre_paciente = ?,  direccion_paciente = ?, eps = ?, nombre_acompanante = ?, celular_acompanante = ?, historial_medico = ?, detalle_historial = ? WHERE id = ?";
$stmt1 = mysqli_prepare($con, $query);
if ( !$stmt1 ) {
  die('mysqli error: '.mysqli_error($con));
}
mysqli_stmt_bind_param($stmt1, 'sssssssi',$Nombre_paciente, $Direccion_paciente, $Eps, $Nombre_acompanante, $Celular_acompanante, $Historial_medico, $Detalle_historial,$Id);
